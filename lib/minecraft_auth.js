...
async function getMsaAccessToken(cookieFile, proxy) {
  ...
  for (let hop = 0; hop < 25 && !code; hop++) {
    ...
    // Auto-submit the consent / DoSubmit form. The Microsoft "Let this app
    // access your info?" page has a "Yes" button (not always a DoSubmit JS).
    const isConsent = /ppsecure|ConsentForm|idBtn_Accept|value="Yes"|value="yes"/i.test(body);
    if (body.length < 30000 && (body.includes('DoSubmit') || isConsent)) {
      const formUrl = getUrlPost(body);
      if (formUrl) {
        const fields = { login: '', passwd: '' };
        const inputRx = /<input[^>]*name="([^"]+)"[^>]*value="([^"]*)"/g;
        let m;
        while ((m = inputRx.exec(body)) !== null) {
          if (!(m[1] in fields)) fields[m[1]] = m[2];
        }
        const sFT = getFlowToken(body) || fields.PPFT || '';
        if (sFT) fields.PPFT = sFT;
        if (isConsent) fields.Yes = 'Yes';
...
/**
 * Mint the MSA access token for Minecraft via a visible Chromium window
 * (manual consent) — handles the Minecraft app consent the HTTP flow can't.
 */
async function getMsaAccessTokenViaBrowser(cookieFile) {
  const rows = parseNetscapeCookieFile(cookieFile);
  ...
  try {
    ...
    const cookies = rows.map((r) => ({
      name: r.name,
      value: r.value,
      domain: String(r.domain).replace(/^\./, ''),
      path: r.path || '/',
      secure: !!r.secure,
      httpOnly: false,
      sameSite: r.secure ? 'None' : 'Lax',
      ...(r.expires > 0 ? { expires: r.expires } : {}),
    }));
    ...
    const authorizeUrl =
      ...
      `&scope=${encodeURIComponent(SCOPE)}&response_type=code` +
      `&redirect_uri=${encodeURIComponent(REDIRECT_URI)}&display=touch&locale=en`;
    await page.goto(authorizeUrl, { waitUntil: 'domcontentloaded', timeout: 30000 }).catch(() => {});
    console.log(`  [ssid] MS sign-in window open — approve/accept if prompted (${path.basename(cookieFile)})`);
    ...
    while (Date.now() < deadline) {
      ...
      for (const sel of consentSelectors) {
        try {
          ...
          if (await el.count()) { await el.click({ timeout: 2000 }); break; }
        } catch (_) { /* keep waiting */ }
      }
      await new Promise((r) => setTimeout(r, 500));
    }
    if (!code) throw new Error('OAuth did not produce a code (manual consent required)');
    const tokenBody = new URLSearchParams({
      client_id: MINECRAFT_CLIENT_ID,
      code,
      grant_type: 'authorization_code',
...
async function mintMinecraftToken({ cookieFile, proxy = null }) {
  // 1. MSA access token from cookies (browser for manual Minecraft-app consent)
  const msaAccessToken = await getMsaAccessTokenViaBrowser(cookieFile);
