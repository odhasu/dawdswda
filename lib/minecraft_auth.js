  ...
  for (let hop = 0; hop < 25 && !code; hop++) {
    ...
    if (body.length < 20000 && body.includes('DoSubmit')) {
      ...
      if (formUrl) {
        ...
        res = await axios.post(formUrl, postBody, {
          headers: {
    ...
    try {
      ...
      if (parsed.hostname.includes('privacynotice')) {
        ...
        if (ru) { url = ru; res = await axios.get(url, { headers: { 'User-Agent': UA, Cookie: cookieHeader }, ...axOpts, maxRedirects: 0, validateStatus: () => true, timeout: 30000 }); continue; }
      }
...
module.exports = { mintMinecraftToken, SCOPE, REDIRECT_URI };
